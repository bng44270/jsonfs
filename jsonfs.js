...
class JsonFS {
  ...
  getDirectoryObject(path) {
    ...
    if (path) {
      pathAr = path.replace(/\/$/,'').split('/').slice(1);
    }
    else {
      pathAr[0] = '';
    }
    ...
    if (pathAr[0].length > 0) {
      pathAr.forEach(p => {
        dirPtr = (dirPtr['type'] == 'dir') ? dirPtr['children'] : dirPtr;
  ...
  getDirectoryListing(dirPath) {
    ...
    return (directoryObj.type == 'dir') ? Object.keys(directoryObj.children).map(e => {
      if (directoryObj['children'][e].type == 'file') return e;
      if (directoryObj['children'][e].type == 'dir') return (e + '/');
    }) : [];
  }
  ...
  getFileAsBlob(filePath,mimeType='text/plain') {
    ...
    return (fileObj.type == 'file') ? (new Blob([atob(fileObj.content)],{type:mimeType})) : (new Blob());
  }
  
  getFileAsString(filePath) {
    var fileObj = this.getDirectoryObject(filePath);
    
    return (fileObj.type == 'file') ? atob(fileObj.content) : '';
  }
}